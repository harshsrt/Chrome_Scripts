...
public class DetailedLinkReport {

	@Test
	public void Detailed() throws EncryptedDocumentException, InvalidFormatException, IOException {
	
		FileInputStream fis=new FileInputStream("C:/Selenium/Testdata/Global.xlsx");
		Workbook wb=WorkbookFactory.create(fis);
		Sheet s=wb.getSheet("DetailedLinkReport");
		s.createRow(0);
...
List<String> external = new ArrayList<String>();
		...
		Assert.assertFalse(id.contains("No ID Found"));
		Assert.assertFalse(mscom.contains("No"));
		Assert.assertFalse(https.contains("No"));
	}
